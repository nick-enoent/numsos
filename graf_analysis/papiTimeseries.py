from grafanaAnalysis import papiAnalysis
from numsos.DataSource import SosDataSource
from numsos.Transform import Transform
from sosdb.DataSet import DataSet
from sosdb import Sos
import pandas as pd
import numpy as np
import time

class papiTimeseries(papiAnalysis):
    def __init__(self, cont, start, end, schema=None, maxDatapoints=4096):
        """Return time series data for papi-events schema"""
        self.start = start
        self.end = end
        self.schema = schema
        self.maxDataPoints = maxDataPoints
        self.src = SosDataSource()
        self.src.config(cont=self.cont)
        self.result = []

    def get_data(self, metrics, job_id, params):
        if self.schema == 'kokkos_app':
            for metric in metrics:
                src.select([metric, 'start_time'],
                    from_ = [ self.schema ],
                    where = [
                        [ 'start_time', Sos.COND_GE, start ]
                    ],
                    order_by = 'job_id'
                )
                res = src.get_results()
            return res
        elif schema == None:
            print("Please provide valid schema")
            return None
        try:
            if not job_id:
                return [{ "target" : "Job Id required for papi_timeseries", datapoints : [] } ]
            xfrm, job = self.derived_metrics(job_id, time_series=True, start=start, end=end)
            for metric in metrics:
                if metric in self.event_name_map:
                    metric = self.event_name_map[metric]
                datapoints = []
                i = 0
                while i < len(job.array(metric)):
                    if i > 0:
                        if job.array('rank')[i-1] != job.array('rank')[i]:
                            result.append({"target" : '[Rank'+str(job.array('rank')[i-1])+']'+metric,
                                           "datapoints" : datapoints })
                            datapoints = []
                    nda = np.array(job.array('timestamp'), dtype='double')
                    dp = [ job.array(metric)[i], nda[i]/1000 ]
                    datapoints.append(dp)
                    i += 1
                result.append({"target" : '[Rank'+str(job.array('rank')[i-1])+']'+metric,
                               "datapoints" : datapoints })
            return result
        except Exception as e:
            a, b, c = sys.exc_info()
            log.write('papi_timeseries '+str(e)+' '+str(c.tb_lineno))
            return None


